...
| [`MTM-CORE.md`](./MTM-CORE.md) | **2.4 specification (the spine).** One unified 0→6 lifecycle: Classify → Ground → Escalate → Contract → Implement → Self-check → Verify. The three docs below are retained as phase-level detail. ⚠️ **Written in Traditional Chinese** — English readers should use the 2.0 article and `MTM-LITE.md`; a translation is outstanding. |
