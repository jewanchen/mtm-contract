...
### #21 — A subtractive change is reviewed for what it removed, never for what it removed **too much of**  `pending-signoff`
...
- **Recommendation: do not promote now.** Park it, or hold it as a ledger data point until a second instance appears. If it is parked, a decidable graduation condition would be: **one further case where a subtractive change silently removes a capability beyond its stated target, and the review's framing is what missed it.** Two instances make the asymmetry arguable; one makes it an anecdote.
