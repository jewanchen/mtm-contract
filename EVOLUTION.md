...
**2026-08-07 · A production outage reported from outside — and the proposal it did not justify** — T1 · ✅ · 0

**The second field report from the same outside agent**, five days after the first. An enterprise onboarding outage in NiceMeet: a security batch removed an over-exposed invitation code from an unauthenticated endpoint, and while writing the comment above the property, deleted the `@Column` line directly beneath it. The property survived, its persistence did not — codes were still generated and still emailed, so the employee held a code that looked correct while the stored row was `NULL`. Neither code review nor the batch's independent audits caught it: **everyone asked "is the sensitive value still leaving?", nobody asked "can it still be stored?"** The reporter proposed a rule for it (**#21**) and then argued against its own proposal — one instance, and invariant 6 arguably already covers it. **Parked, not promoted.**

**The executor's own error in this case is the part worth keeping.** Handling an unrelated plugin version bump, it ran `git add -A` and swept the reporter's uncommitted #21 into a commit whose message reads `chore(plugin): version the plugin at 2.5.0` — a governance proposal entering the ledger under a message that says only "semver". Caught when the person asked what the uncommitted change was, by which point it was already pushed. **Same shape as the finding it was carrying**: an operation framed by its intent (bump a version) silently doing more than its frame described, and the review — the commit message — written from the frame rather than from the diff. Not corrected by rewriting history; corrected by appending the truth, since an erased record and an honest one do not read the same.
...
### #21 — A subtractive change is reviewed for what it removed, never for what it removed **too much of**  `⏸ parked-pending-evidence`

- **Source**: field report from the same outside agent as #20 — a production outage found in NiceMeet on 2026-08-07, root-caused by that agent, fixed by the project's main line.
- **Evidence (one instance, stated plainly as one)**: a security batch closed an over-exposure — a 6-digit invitation code was being returned by an unauthenticated endpoint. While writing the "this column must never leave an unauthenticated endpoint" comment above the property, the edit also deleted the line directly beneath it: `@Column({ type: 'varchar', length: 6, nullable: true })`. The property survived; its persistence did not.
  - Consequence: the code was still generated, still emailed, so **the employee holds a code that looks correct** — but the INSERT no longer carried the column, so the row stored `NULL`, and the accept path's `if (!inv.acceptCode)` fired for **every** invitation. Enterprise employee onboarding was 100% broken in production, and the error text told the admin to *re-send*, which reproduced it exactly. A paying customer sat in that loop.
  - Neither code review nor the batch's independent audits caught it. **Everyone was asking "is the sensitive value still leaving?" Nobody asked "can it still be stored?"**
- **The claimed general shape**: a subtractive change (remove / hide / restrict / stop logging) carries a **symmetric risk — removing more than intended** — but its review is framed by the removal's *intent*. The finding "you still expose X" is cheap to state; "you also destroyed X's write path" requires exercising a path nobody was asked to look at. So the over-removal is systematically invisible, in a way the under-removal is not.
- **Honest self-check against the two tests — I do not think this clears the bar yet:**
  1. **Substantive** — arguably yes: it is a structural asymmetry in how subtractive changes get reviewed, and it belongs to the same family of symmetric-gap findings as 6/8 and #20.
  2. **General across tasks** — **not demonstrated.** This is **one** event. §D is explicit: never add a rule for a single event.
- **The objection that may kill it — invariant 6 might already cover this.** Had anyone *executed* the path (create an invitation, then look at the stored row) instead of reading the diff, it would have surfaced immediately. On that reading this is not a missing rule but an un-followed one, and adding a rule would be the per-incident patching §D warns against.
  - The one thing invariant 6 does *not* say is **which** paths a subtractive change owes execution to. The batch's own regression list did name `邀請寄送` — the **write** side — and stopped there; the **read** side, where the deleted column is consumed, was never listed. So the residue, if any, is narrow: *when a change touches a field, the regression list owes that field's whole lifecycle — write, store, read, consume — not just the surface that was edited.*
- **Decided 2026-08-07: parked, not promoted.** The objection above is what carried it — on the evidence available this is an un-followed invariant 6, not a missing rule, and §D forbids adding a rule for a single event. **Graduation condition**: one further case where a subtractive change silently removes a capability beyond its stated target, **and the review's framing is what missed it**. Two instances make the asymmetry arguable; one makes it an anecdote.
  - Noted against the "expensive and vague" objection, for whoever picks this up second: **for subtractive changes specifically, "touched" is mechanically decidable — it means "appears in the diff".** That is narrower than the general rule the reverse-risk bullet costs out, and it is the form to test first if a second instance arrives.
- **Reverse risk of promoting early**: the natural rule — "for subtractive changes, exercise the full lifecycle of every field touched" — is expensive and vague at the edges (what counts as touched?), and a rule that is expensive and vague gets skipped, which is worse than no rule because it looks like coverage. #16 is precisely about that failure.
