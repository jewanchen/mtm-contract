...
**Specification version:** 2.0 at publication; currently **2.2**, which adds the invariant described in §9, a debug branch for symptoms whose scope is not yet known, and the rule that a gate is only hard if something mechanically enforces it. One lifecycle, a greenfield branch, and an evolution engine. Reference implementation: [`MTM-CORE.md`](./MTM-CORE.md) (currently in Traditional Chinese; English readers start with [`MTM-LITE.md`](./MTM-LITE.md))
